...
window.addEventListener('DOMContentLoaded', () => {
    ...
    //     console.log(target.contains(document.querySelector('.modal__dialog')));
    //     if (target.contains(document.querySelector('.modal__dialog'))) {
    //         modal.classList.add('hide');
    //         modal.classList.remove('show');
    //         document.body.style.overflow = '';
    //     }
    ...
    function delelteNotDigits(str) {
        return +str.replace(/\D/g, '');
    }

    next.addEventListener('click', () => {
        if (offset == delelteNotDigits(width) * (slides.length - 1)) { // '500px'
            offset = 0;
        } else {
            offset += delelteNotDigits(width);
        }
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = delelteNotDigits(width) * (slides.length - 1);
        } else {
            offset -= delelteNotDigits(width);
        }
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            slideIndex = slideTo;
            offset = delelteNotDigits(width) * (slideTo - 1);
